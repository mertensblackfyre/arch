...
Singleton {
    ...
    readonly property var available: {
        var r = []
        for (var i = 0; i < networks.length; i++)
            if (!networks[i].inUse) r.push(networks[i])
        return r
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: function(line) {
                ...
                root.networks = nets.slice()  // force new array reference
            }
        }
        ...
        onExited: function(code, status) {
            if (code === 0) {
                ...
                if (err.indexOf("secret") >= 0 || err.indexOf("password") >= 0) {
                    var np2 = Object.assign({}, root.needsPassword)
                    np2[connectProc._ssid] = true
                    root.needsPassword = np2
    ...
    function scan(rescan) {
        if (scanning || !root.wifiEnabled) return
